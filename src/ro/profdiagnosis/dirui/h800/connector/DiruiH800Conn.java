 ...
 */
package ro.profdiagnosis.dirui.h800.connector;
...
public class DiruiH800Conn implements Runnable, SerialPortEventListener {
    ...
    int cerereID;
    boolean validEntry;
    static final char DIRUI_STX = 0x02;
    static final char DIRUI_ETX = 0x03;
    static final char DIRUI_LF = 0x0a;
        ...
        while (portList.hasMoreElements()) {
            ...
            if (portId.getPortType() == CommPortIdentifier.PORT_SERIAL) {
                if (portId.getName().equals(commNo)) {
                    DiruiH800Conn reader = new DiruiH800Conn();
                }
            }
        }
    ...
    public DiruiH800Conn() {
        try {
            serialPort = (SerialPort) portId.open("DiruiH800Conn", 2000);
        ...
        readThread.start();
        System.out.println("[DIRUI H800] Astept rezultate ...");
    }
    ...
    private String getParamID(String diruiParamCode) {
        String paramID = "";
        switch (diruiParamCode) {
            case "BLD":
                paramID = "26"; break;
            case "LEU":
                paramID = "31"; break;
            ...
            case "SG":
                paramID = "29"; break;
            case "VC":
                paramID = "555"; break; // ToDo
            case "MALB":
                paramID = "666"; break; // ToDo
            default:
                System.out.println("Parametru necunoscut [" + diruiParamCode + "]");
        }
        //System.out.println("cod="+diruiParamCode+", id="+paramID);
        return paramID;
    }
            ...
            case SerialPortEvent.DATA_AVAILABLE:
                ...
                try {
                    while ((c = inputStream.read()) != -1) {
                        ...
                        if (c == DIRUI_LF) {   /* linie noua */
                            readBuffer.append((char) c);
                            if (!readBuffer.toString().isEmpty()) {
                                System.out.print(readBuffer);
                                if (readBuffer.toString().startsWith("ID")) {
                                    validEntry = true;
                                    try {
                                        cerere = readBuffer.substring(2).trim();
                                    ...
                                    paramCode = readBuffer.substring(1, 5).trim();
                                    rezultat = readBuffer.substring(5).trim();
                            ...
                            if (c == DIRUI_ETX) {
                                if (!validEntry) {
                                    System.out.println("EROARE: ID-ul cererii nu exista in baza de date. "
                                            + "Verifica daca ai introdus corect ID-ul in analizor!!!\n\r");
                                }
                                validEntry = false;
                            }
                            readBuffer.append((char) c);
